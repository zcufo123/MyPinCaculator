...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val minLength = application.resources.getInteger(R.integer.pin_min_length)
    private val maxLength = application.resources.getInteger(R.integer.pin_max_length)
    private val formatter = ISO3Formatter()

    fun enterPin(pin: String)  {
        if (pin.length < minLength) {
            output.postValue(getApplication<Application>().resources.getString(R.string.pin_too_short))
            return
        }
        if (pin.length > maxLength) {
            output.postValue(getApplication<Application>().resources.getString(R.string.pin_too_long))
            return
        }
